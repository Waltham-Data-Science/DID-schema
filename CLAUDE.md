...
RE-DERIVED 2026-09-22 (#67 calculator restructure + sibling drift, all 8 nouns; run `python3 tools/check_prose_counts.py` to reproduce): the #67 landing added `calculator` + `runtime_environment` + the `tuning_curve` family (5 markers + composite) + the `tuning_curve_calculation` family (6 leaves + abstract) + `reverse_correlation` + `hartley_reverse_correlation`; NDI `origin/main` also gained 11 new templates (gene-expression family + demoNDISeries pair + fileReference); DID-matlab main gained `resolveClockAlignment` and other files. The live figures are: 272 .m file(s) under DID-matlab src/; 113 v1 ledger rows in the V_eta coverage ledger; 102 NDI templates on origin/main; 1,179 .m files (was 1013; NDI main advanced significantly since); 254 distinct V_eta class names; 260 json file(s) under `schemas/V_eta/`; classes compared: 68 (in `check_tombstones.py --enforce`); **11 deleted** (in `_DELETE_PHASE8`). The +11 NDI templates are: cellTypeLabels, demoNDISeries, demoNDISeriesMixed, fileReference, geneExpression, geneList, geneListMapping, spatialGeneExpressionCells/Pyramid/Tiles. Re-derive with `check_prose_counts`, do not quote these numbers directly.


RE-DERIVED 2026-09-23 (#70 spatial-transcriptomics family landed + sibling drift; `check_prose_counts` all nouns): commit `71298fd` (DID-schema PR #64) minted 8 new V_eta classes + the pyramid reshape; DID-matlab veta-branch gained the 8 sibling migrators (Session 2 commit `932d653`, PR #151). Current values, re-derived by `python3 tools/gates.py --check` locally against both sibling checkouts on `claude/v-eta-migration-plan-35jj1z`: **280 .m file(s) under DID-matlab src/** (was 272 on 2026-09-22); **93 migrator .m file(s)** in `+migrators_j` (was 85; +8 from Session 2 commit `932d653`); NDI templates on origin/main; 1,180 .m files (was 1,179; routine drift; `git ls-tree -r origin/main | grep -c \.m$` = 1180); **102 NDI templates** on origin/main (unchanged); **113 rows in the V_eta coverage ledger** (unchanged from 2026-09-22 -- the +8 spatial-transcriptomics rows were counted in that number); **262 distinct V_eta class names** (was 254; +8); **268 json file(s) under `schemas/V_eta/`** (was 260; +8); **42 direct subclasses** of `data_type` (unchanged); the registry has **39** rows (was 38; +1 NCIT:C16608 binding); tombstones classes compared: 76 (was 68; +8); **11 deleted** in `_DELETE_PHASE8` (unchanged); **55 plan documents** in `schemas/*.md` (unchanged). Re-derive with `check_prose_counts`, do not quote these directly.


RE-DERIVED 2026-09-23 (didmatlab_m_files, sibling drift; `check_prose_counts` was the only red step with the siblings on their V_eta branch): **291 .m file(s) under DID-matlab src/** (`cd DID-matlab && find src -name '*.m' | wc -l` = 291 at `47cf8ba` on `claude/v-eta-migration-plan-35jj1z`; was 280 on 2026-09-23 earlier). Only the denominator moved; nothing in this repository changed. Re-derive with `check_prose_counts`, do not quote.

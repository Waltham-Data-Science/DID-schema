# V_alpha_SPEC.md — DID/NDI Document Schema Format (V_alpha)

## Purpose

This document is a complete specification for the **V_alpha** schema set in the
`did-schema` repository. V_alpha defines the canonical JSON schema format for
DID/NDI document types using a **flat directory layout**: one JSON file per
document type at the top of `schemas/V_alpha/`.
...
A revised schema set (**V_beta**) layered on top of V_alpha adds snake_case
naming requirements; see `schemas/V_beta_SPEC.md`.
...
- Ships one JSON file per document type under `schemas/V_alpha/` using a flat layout.
...
│
├── schemas/
│   ├── V_alpha_SPEC.md                 ← (this file) V_alpha specification
│   ├── V_alpha_notes.md                ← V_alpha status and deprecated types
│   └── V_alpha/                        ← flat directory of V_alpha schemas
...
Each document type is a **single JSON file** at the top of `schemas/V_alpha/`,
named `<classname>.json`. The filename stem must equal the document's
`_classname` value. There is exactly one schema file per document type;
subdirectories and per-type directories are not used.

The meta-schema (`did_schema_meta.json`) lives alongside the document-type
schemas in `schemas/V_alpha/`.
...
| `_schema`    | string | yes      | Path or token-substituted path to the parent schema file. Under the flat V_alpha layout this is `$NDISCHEMAPATH/<parent_classname>.json`. |
...
The `directory` document type (`schemas/V_alpha/directory.json`) is the storage
...
`schemas/V_alpha/did_schema_meta.json` is a JSON Schema Draft 7 file (standard JSON
...
### `schemas/V_alpha/probe_location.json`

...
RE-DERIVED 2026-09-22 (#67 calculator restructure + sibling drift, `check_prose_counts`): 113 v1 ledger rows in the coverage ledger; classes compared: 68 in check_tombstones.py; 254 distinct V_eta class names; 260 json file(s) under schemas/V_eta; 102 NDI templates on origin/main; 1,177 .m files. Re-derive rather than quote; #67 added `calculator` + `runtime_environment` + the `tuning_curve`/`tuning_curve_calculation` families (11 new classes total), and NDI `origin/main` gained 11 templates (gene-expression family + demoNDISeries pair + fileReference).

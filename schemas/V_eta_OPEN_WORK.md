...
| 117 | **A SERIES OF EVENT TIMES HAS NO CLASS TO BE. Two signed plans route the per-neuron spike trains to a `sampled_body` on a spike-time observation, and that observation names an ABSTRACT class.** Opened 2026-08-17 out of the #60 Q1 amendment, which decided the DESTINATION and deliberately did not decide this. | `V_eta_ensemble_plan.md:10` (signed 2026-08-06) names the host *"a spike-time `subject_observation` of that neuron"*. **`subject_observation` is abstract** -- `+did2/+schema/cache.m` raises `did2:validation:abstractInstantiation` for any document naming it -- and nothing concrete under it fits:<br><br>`DENOMINATOR: 249 json file(s) under schemas/V_eta/ read`<br>`  classes ending _observation : 33`<br>`  any of them for event TIMES :  0`<br><br>**`count_observation` is the near miss and taking it would be a unit error, not a shortcut.** `jrclust_clusters` already folds to it for a per-spike integer LABEL series (`count_observation` + `sampled_body` + anchor), so the neighbourhood is right and the quantity is not: a spike TIME is not a count. This is the same species as the `fitcurve` SSE/goodness-of-fit swap and `pyraview`'s *"the physical quantity is not carried on the doc"* -- asserting a dimension the source never stated. **AND THE ANSWER CANNOT BE SPIKE-SPECIFIC.** `element_epoch`'s `.vhsb` is a GENERIC `(timepoints, datapoints)` series -- `+ndi/+element/timeseries.m:274` writes both, for ANY derived element type -- so whatever lands must key on element `type` the way `jRecordingModality` already does, or the next derived type silently inherits a spike-shaped home. Note `'spikes'` is not in that map today (ONE hit, a comment at `:157`), so a map row is part of this too. **WHAT WOULD SETTLE IT:** a team call on whether event times get their own `data_type` + leaf (the T12 *"when is a new data_type warranted"* test), or whether the signed `axes[]` model already covers it -- a `sampled_body` whose TIME axis is irregular/enumerated and whose values are the marks, which would make the observation's quantity the MARK rather than the time. The second reading needs no new class and is not obviously wrong; it is not proposed here because choosing between them is a disposition. **BLOCKS:** `V_eta_ensemble_plan.md` deferred build task 1, and with it the #60 `element_epoch` dissolution -- whose INTERIM (acquisition_epoch keeps carrying the 252) is signed and needs nothing. |

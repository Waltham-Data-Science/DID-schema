...
| #A | **spatialGeneExpressionPyramid schema reshape** — declare `⊂ [geneExpression, subject_observation]` (was `⊂ [base, geneExpression]`). Move the existing `subject_id` required-ness from the class's own declaration onto the inherited `subject_observation` slot. Add the direction's `variable` / `method_parameters` / `sample_time` / `time_reference_#` slots. Blocks on **#B** (variable binding term) for validation. Ref: `V_eta_go_forward_class_audit.md` TEAM-SIGN-OFF `[spatial_transcriptomics_family]` 2026-09-22; Waltham-Data-Science/DID-schema#70. |
| #B | **Bind a `variable` term for the pyramid observation** — add a controlled term for "gene expression" (or the specific spatial-transcriptomics variable) to NDIC or the binding registry. Prerequisite for **#A** end-to-end validation. Ref: same. |
| #C | **Import the spatial-transcriptomics family as v1 sources** — 7 classes: `spatialGeneExpressionPyramid`, `spatialGeneExpressionCells`, `spatialGeneExpressionTiles`, `geneListMapping`, `cellTypeLabels`, `fileReference`, `geneList`; plus `geneExpression` as a `⊂ base` shape mixin. Update `_disposition` markers in `build_v_eta.py` so they persist structurally. Ref: same. |
| #D | **V_eta migrators for the spatial-transcriptomics family** — 8 `+migrators_j/*.m`: mostly id-preserving passthroughs; the `spatialGeneExpressionPyramid` migrator carries the observation direction reshape (may need a `jGeneExpressionObservation` helper analogous to `jCalculation`). `fileReference` is a distinct migrator, NOT the `foldGenericFiles` batch pass — the two classes coexist by design. Ref: same. |
| #E | **Fast-fixture tests for the spatial-transcriptomics migrators** — one fixture per migrator; assert id preservation, subject_id inheritance chain (Cells/Tiles reaching pyramid's subject), pyramid emitted with the subject_observation direction. Corpus proof deferred to **#F**. Ref: same. |
...
| #G | **Follow-on refactors when 2.D data_body ships** — `spatialGeneExpressionTiles` and `spatialGeneExpressionCells` are strong candidates for `axes[]` / `format` / `compression` migration once the data_body plan lands. Not blocked on anything besides the data_body plan itself. Ref: same, plus `V_eta_data_body_model_plan.md`. |
| #H | **T8 binding follow-ons for the spatial-transcriptomics family** — candidate controlled terms: `contour_reference` (currently enum), `segmentation_method`, `mapping_type` (`alias`/`ortholog`), `taxonomy_level`, `assignment_method`, `checksumAlgorithm`, `formatOntology`, `origin_corner`. Not blockers for PERSIST; scheduled as the family's binding governance pass. Ref: same. |

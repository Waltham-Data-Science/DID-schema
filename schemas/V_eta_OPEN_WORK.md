...
| 71 | **`openminds_stimulus` loses its referent — 635 documents** | opened 2026-08-08. NDI's template AND writer both say `stimulus_element_id` (`openMINDSobj2ndi_document.m:58`); V_eta declares `stimulus_id` `mustBeNonEmpty`; `migrators_j/openminds_stimulus.m:50` looks for `stimulus_id`/`element_id`/`subject_id`, none present; `jCarrySubject.m:30` returns `''` with no guard (the guard is local to `ontology_table_row.m:650`); `universalRenames`'s `renameDependsOnEntries` normalises the SHAPE only, never the name. So all 635 migrate to a `term_assertion` with `subject_id = ''` — a further instance of the invented-empty-edge pattern, absent from the `CLAUDE.md` list. `testFixtureCorpus.m:1036` builds the fixture with OUR wrong name, so the test cannot catch the code. FIX: read `stimulus_element_id`, correct the tombstone, invert the fixture. The 635 is the walkthrough histogram (119,166 docs, JH/Dab/B), NOT a fresh census. Full write-up: `V_eta_openminds_family_record.md` Part 7. |
...
| 74 | **Build deferred: the settings model — `method_parameters` as a document + a bound `parameter` entry** | opened 2026-08-09, `V_eta_method_parameters_plan.md` "FINAL MODEL". The four spike-app settings classes fold into ONE generic `method_parameters` document (id + `base.name` preserved; optional `software_id`/`subject_id`/`epoch_id -> epoch`/`overrides_id`), and `subject_interaction` gains an optional `method_parameters_id`. The settings SHAPE is a `parameter[]` entry modelled on the `axis` entry — identity in a BOUND `variable`, no `unit` field and no `data_type` field — named `parameters` in BOTH mount points (the inline `subject_interaction.method_parameters` field is RENAMED to `parameters`, following `axes`), used BOTH inline and in the document, so a calculator's knobs and a spike app's knobs become queryable by the same mechanism. Routing is per class: the source gave it a name+id and things point at it → document; otherwise inline. FORBID BOTH on one statement (team, 2026-08-09). GATED on the registry carrying dimension + canonical unit (same extension the axis entry needs — the registry has 5 bindings today, none with a dimension). Supersedes the typed-blocks shape and the three rejected class names. |

...
TEAM-SIGN-OFF [stranded sources]: jess@walthamdatascience.com, 2026-08-11 -- generic_file folds to term_observation + opaque_body; valid_interval decomposes to boolean validity_observation + relative_reference, one per interval, anchored by a shared time reference; imageCollection is a tombstone. All three are BUILT and ran in 6 of 6 corpora in run 31522068566. WRITTEN BY CLAUDE AT THE TEAM'S EXPLICIT INSTRUCTION ("Please write the sign off for me. I give you permission this time."), which is a one-time authorisation and not a precedent: Operating Rule 4 otherwise forbids Claude from adding this line, and the next one is the team's to write.

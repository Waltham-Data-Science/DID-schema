# Conversion: did_v1 → V_eta — `contrast_tuning`

## Identity

- **V_eta `class_name`:** `contrast_tuning`
- **V_eta tier:** `stable`
- **V_eta schema path:** `schemas/V_eta/stable/contrast_tuning.json`
...
all field-level typing in V_eta is recovered from the paired db_doc
...
- **Naka-Rushton fit grouping.** did_v1 stores the three model variants
  ...
  names. V_eta groups them under a `fit` structure containing three
...
- **Scalar metrics stored as 1-element arrays.** Several did_v1 fields
  ...
  V_eta — the V_eta schema declares them as `type: double` /
...
- **`sensitivity` is genuinely a vector, not a scalar.** The did_v1
  ...
  sensitivity at i standard deviations of the control response. V_eta
...
- **RB-specific `saturation_index`.** The RB form has 2 free parameters
  ...
  under the RB fit. V_eta encodes that note in the field documentation
...
- **After (V_eta):** an example will be added to
  `schemas/V_eta/examples/contrast_tuning_minimal.json` in a follow-up

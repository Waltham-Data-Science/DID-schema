# Conversion: did_v1 → V_eta — `hartley_calc`

## Identity

- **V_eta `class_name`:** `hartley_calc`
- **V_eta tier:** `stable`
- **V_eta schema path:** `schemas/V_eta/stable/hartley_calc.json`
...
| (top-level) `files.file_list: ["hartley_results.ngrid"]` | top-level `file` array, one entry `name: "hartley_results.ngrid"` | structural rename: did_v1 `files.file_list` → V_eta top-level `file` |
...
- **`files.file_list` → top-level `file`.** did_v1 carries a class-block
  `files.file_list` array; V_eta has a dedicated top-level `file` field
...
`X_sampling: 1`, `Y_sampling: 1`. V_eta preserves these as

> **V_eta retarget (D5).** → a `term_observation` about the imaged element/subject (region term as `value`, spatial-relation `variable`); any image file → an `opaque_body`/`sampled_body`.

# Conversion: did_v1 → V_eta — `ontology_image`

## Identity

- **V_eta `class_name`:** `ontology_image`
- **V_eta tier:** `stable`
- **V_eta schema path:** `schemas/V_eta/stable/ontology_image.json`
...
(`ontology_name` + `ontology_region`); V_eta collapses them into a
...
- **Collapse two coordinated chars into one `ontology_term`.** Same
  ...
  human-readable region label) merge into V_eta's `region`
  composite:
  ...
  V_eta's schema declares this field `mustBeNonEmpty: true`. Migrated
...
between did_v1 and V_eta; only the underscore-prefix stripping
...
- **TODO-domain:** the V_eta schema requires `region` to be
  ...
  the V_eta constraint as written. Options: (a) tighten by demoting
...
- V_eta schema file: [`schemas/V_eta/stable/ontology_image.json`](../../stable/ontology_image.json)

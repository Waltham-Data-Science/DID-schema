# Conversion: did_v1 → V_eta — `hartley_reverse_correlation`

## Identity

- **V_eta `class_name`:** `hartley_reverse_correlation`
- **V_eta tier:** `stable`
- **V_eta schema path:** `schemas/V_eta/stable/hartley_reverse_correlation.json`
...
| `hartley_reverse_correlation.hartley_numbers` | same | `matrix<double>` (integer-valued; integer subtype not introduced for V_eta) |
...
800, 600]`). V_eta preserves those as the `default_value` of each
field.

...
| **R5** infra naming smells → RENAME (T11/T13) | FINAL | ✅ **4 of 5 BUILT**: `binaryseries_parameters`→`acquisition_layout`, `dataseries_channel_map`→`channel_assignment`, `daqreader_epochdata_ingested`→`daqreader_epoch_cache`, `daqmetadatareader_epochdata_ingested`→`daqmetadatareader_epoch_cache`. Turned out **DID-only**, not a cross-repo lockstep: the NDI hits are all v1 WRITERS (they keep writing v1 names) and `+migrate/` has no V_eta-side reader of these strings — same finding as `element_epoch`. ⏳ HELD: `daqreader_image_epochdata_ingested` — dropping `_epochdata_ingested` is decided, but whether `_image` is a distinct cache SHAPE or a modality label (→ fold + modality field) is a factual question for NDI, and half-renaming it is exactly what R5's re-audit warned against. |

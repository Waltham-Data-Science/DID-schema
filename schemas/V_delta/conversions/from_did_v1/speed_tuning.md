# Conversion: did_v1 → V_delta — `speed_tuning`
...
Result type for empirical and fit-derived tuning along (SF, TF) pairs in the speed plane. Independent of the calculator that produced it; calculators (e.g., `speed_tuning_calc`) inherit from this class.
...
| `speed_tuning.fit*` families | nested `structure` per fit family | scalar metrics → `double`; array metrics → `matrix<double>`; fit families: `fit (Priebe)`, `fit_no_speed`, `fit_fullspeed` |
...
- Calculator(s) that produce this result type: [`speed_tuning_calc.md`](./speed_tuning_calc.md)

...
TEAM-SIGN-OFF [spike train leaf]: jess@walthamdatascience.com / 2026-08-17 -- a per-neuron spike train is a `time_observation` (the leaf formerly `duration_observation`; see TEAM-SIGN-OFF [time dtype], V_eta_tenet_audit.md, same day) with `subject_statement.variable` = spike. The SPIKE TIMES ARE THE VALUE -- an array of `time` cells, each an offset from the statement's own anchor (`time_reference_1` -> a `relative_reference` -> the minted `epoch`) -- and the body's axis is a bare regular index. NO NEW CLASS is minted anywhere. This closes deferred build task 1 of this plan ("confirm the element migrator already lands this for 'spikes' elements; if not, add it" -- the confirmation ran 2026-08-17 and the answer is no) and it closes V_eta_OPEN_WORK.md row #117.

WHY THE TIMES ARE THE VALUE AND NOT THE AXIS, since the plan's own text says "event times to a sampled_body" and both readings fit that phrase. The alternative was `logical_observation` -- times as an irregular time AXIS, value an array of N `true`. Both encodings carry exactly one uninformative slot, but they do not cost the same: a regular index axis is `origin + spacing + n`, three numbers regardless of N, while N booleans is O(N) on every spike of every neuron. The precedent settles the rest: `relative_reference.value.start` is an anchor OFFSET typed with this same composite, so "an offset whose anchor is named elsewhere on the document" is a shape the model already uses, not a new one.

WHAT WAS REJECTED, and why it is recorded rather than dropped -- Claude proposed minting an `event` data_type or an undimensioned `event_observation`, TWICE, and both violate T12's first escape and the rule stated in TEAM-SIGN-OFF [logical_observation] (2026-08-12): a data_type names the KIND of value, `variable` names what it is about. "Event" is a kind of thing. The tell was available and missed both times: nobody could say what `event.value` would hold.

ACCEPTED COST, stated rather than discovered later: the ensemble's combined marked point process stores `(times, neuronIndex)` with times on the AXIS and the index as the value, because there the mark is real. So the source (times in the value) and the derived cache (times on the axis) use different encodings, and the "losslessly inter-convertible, merge-with-labels / group-by-id" relationship this plan signs is a RE-ENCODING rather than a regroup. That is accepted knowingly: paying O(N) on every archival train to keep a cache conversion symmetric is the worse trade.

NOT COVERED BY THIS SIGNATURE: (a) whether JRCLUST -- which wrote 20211116's 21 neurons and has NO importer in NDI-matlab -- put anything meaningful in the value column of its `.vhsb`; the two in-tree sorters write `ones(size(spike_times))` and NDI's own per-neuron API takes "a vector of spike times", so the model says there is nothing there, but no `.vhsb` was read (0 exist in the authoring container). A JRCLUST train carrying real per-spike amplitudes would be a departure from NDI's model, and would make the value slot genuinely occupied. (b) The BUILD, which is blocked on `axes[]` (#45) exactly as `valid_interval` is.


RE-DERIVED 2026-09-22 (#67 calculator restructure + sibling drift, `check_prose_counts`): 260 json file(s) under schemas/V_eta. Re-derive rather than quote; #67 added `calculator` + `runtime_environment` + the `tuning_curve`/`tuning_curve_calculation` families (11 new classes total), and NDI `origin/main` gained 11 templates (gene-expression family + demoNDISeries pair + fileReference).

RE-DERIVED 2026-09-23 (spatial-transcriptomics family landed via #64 commit `71298fd`, `check_prose_counts`): **268 json file(s) under `schemas/V_eta`** (was 260; +8 spatial-transcriptomics classes). Re-derive with `python3 tools/check_prose_counts.py`, do not quote directly.

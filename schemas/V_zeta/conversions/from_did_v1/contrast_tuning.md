# Conversion: did_v1 → V_zeta — `contrast_tuning`

## Identity

- **V_zeta `class_name`:** `contrast_tuning`
- **V_zeta tier:** `stable`
- **V_zeta schema path:** `schemas/V_zeta/stable/contrast_tuning.json`
...
all field-level typing in V_zeta is recovered from the paired db_doc
...
- **Naka-Rushton fit grouping.** did_v1 stores the three model variants
  ...
  names. V_zeta groups them under a `fit` structure containing three
...
- **Scalar metrics stored as 1-element arrays.** Several did_v1 fields
  ...
  V_zeta — the V_zeta schema declares them as `type: double` /
...
- **`sensitivity` is genuinely a vector, not a scalar.** The did_v1
  ...
  sensitivity at i standard deviations of the control response. V_zeta
...
- **RB-specific `saturation_index`.** The RB form has 2 free parameters
  ...
  under the RB fit. V_zeta encodes that note in the field documentation
...
- **After (V_zeta):** an example will be added to
  `schemas/V_zeta/examples/contrast_tuning_minimal.json` in a follow-up

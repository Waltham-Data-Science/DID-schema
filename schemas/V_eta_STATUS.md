...
The class count is not the work count. 27 open classes are 19 decisions, because most open classes move as a family.
...
## What is actually left on the 27 open classes
...
| build: migrator lines inspected | 29738 |
| build: &nbsp;&nbsp;-- of those, per-document migrator | 20308 |
| build: &nbsp;&nbsp;-- of those, batch post-pass | 9430 |
| build: classes queried | 27 |
...
| `ensemble` | ensemble | (b) | 1 consuming reference(s) (1 per-document migrator); decided target(s) built: `subject`, `directed_relation`, `sampled_body` | - | - | - | 19 (17+2) | n/a -- not measured |
...
| `epochid` | epoch | (b) | 12 consuming reference(s) (11 per-document migrator, 1 batch post-pass) | - | 1 (1+0) | 8 (7+1) | 77 (69+8) | n/a -- not measured |
...
| **file navigation** | 2 | 1 of 1 | filenavigator -> `epoch_file_pattern` (id preserved; patterns PARSED not eval'd); `directory` is not a source | `V_eta_daq_family_decisions.md` |
...
DENOMINATOR: 1 row(s), each searched for its BARE CLASS NAME as a quoted literal in 9 batch post-pass file(s) under `+did2/+convert` (comments stripped).

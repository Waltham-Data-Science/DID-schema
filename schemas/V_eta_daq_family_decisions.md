...
TEAM-SIGN-OFF [daq configuration]: jess@walthamdatascience.com / 2026-08-13 -- `daqreader_ndr` ENDS AS `acquisition_reader` + a `software` entity, and that is the end state. The `ndr` subtype was encoded in the CLASS NAME while `ndi_daqreader_class` already discriminates it, so the migrator de-encodes onto the generic `daqreader` block (`ndr_reader_string` -> `reader_string`, `ndi_daqreader_ndr_class` dropped as redundant, the invented `file_extension` not carried) and HANDS THE BODY ON to migrators_j/daqreader, which performs the fold signed above on 2026-08-08. `daqreader` is an INTERMEDIATE class name and NOT a destination: it appears in the generated `targets` because the derivation follows the call graph and sees both the literal assignment and daqreader.m's guard-1 passthrough, a branch reachable in general and not taken by either PRED document (both carry `ndr_reader_string: 'intan'` AND a populated `ndi_daqreader_class`). `base.id` is PRESERVED, so every `daqreader_id` edge still resolves.
    (Recorded by Claude at the signer's explicit instruction in session_01BenWtpJyRu3QhEZqCErpwm. The signer was shown the generated confirm-sheet row for `daqreader_ndr` -- question, named answer options, the emitted set with `daqreader` marked as an intermediate folded by migrators_j/daqreader.m, and `governance: no signature found` -- and answered "Yes to those 5 rows". The DECISION is the signer's; the transcription is Claude's, and Operating Rule 4 forbids Claude writing such a line unprompted. Corpus state at signing: PRED, 2 documents, corpus-PROVEN in run 31746748355.)
